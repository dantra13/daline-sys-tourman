...
/// <summary>
/// Minimal IDisciplineRegistry fixture used by catalog-coverage tests.
/// Only FBL+M is registered; everything else throws NotImplementedException
/// for members that are never exercised in architecture tests.
/// </summary>
internal static class ArchCatalogRegistry
{
    public static IDisciplineRegistry WithFblM()
    {
        var fbl = DisciplineCode.From("FBL");
        return new FakeRegistry(new Dictionary<DisciplineCode, IReadOnlySet<GenderCode>>
        {
            [fbl] = new HashSet<GenderCode> { GenderCode.M },
        });
    }
